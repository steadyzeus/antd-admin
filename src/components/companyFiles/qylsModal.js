...
const modal = ({
  ...
  const modalOpts = {
    title: `${type === 'create' ? '新建企业流水' : '修改企业流水'}`,
    visible,
    onOk: handleOk,
    onCancel,
  ...
  const props = {
    action: '/InputSystem/DataService/api/v1/upload/qyls/'+item.KeyID,
    listType: 'picture',
  ...
  return (
    <Modal {...modalOpts}>
      <Form horizontal>
        ...
        <FormItem label='对方省市' hasFeedback {...formItemLayout}>
          {getFieldDecorator('DFCity', {
            initialValue: item.DFCity,
            rules: [
              {
                required: false,
                message: '请填写金额'
        ...
        <FormItem label='交易用途' hasFeedback {...formItemLayout}>
