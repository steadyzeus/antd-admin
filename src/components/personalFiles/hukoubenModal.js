...
const modal = ({
  ...
  const modalOpts = {
    title: `${type === 'create' ? '新建户口本信息' : '修改户口本信息'}`,
    visible,
    onOk: handleOk,
    onCancel,
  ...
  const props = {
    action: '/InputSystem/DataService/api/v1/upload/hukouben/'+item.KeyID,
    listType: 'picture',
  ...
  return (
    <Modal {...modalOpts}>
      <Form horizontal>
        ...
        <FormItem label='曾用名：' hasFeedback {...formItemLayout}>
          {getFieldDecorator('OldName', {
            initialValue: item.OldName,
            rules: [
              {
                required: false,
                message: '请填金额'
